  ...
  return (
    <div className="flex-1 flex flex-col min-h-0">
      ...
      <div className="flex-1 overflow-y-auto p-6 flex flex-col gap-6">
        ...
        <div className="bg-white rounded-xl border border-[#DBEFF3] p-4 flex flex-wrap items-center justify-between gap-3">
            ...
            {(["all", "active", "inactive"] as StatusFilter[]).map((s) => (
              <button
                ...
                className={`text-xs font-semibold rounded-full px-3 py-1.5 border transition-colors ${
                  statusFilter === s
                    ? "bg-[#49B0C1] border-[#49B0C1] text-white"
                    : "border-[#ABDBE3] text-[#666666] hover:bg-[#DBEFF3]"
                }`}
              >
        ...
        {loading ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {[...Array(3)].map((_, i) => (
              <div
                key={i}
                className="h-32 rounded-xl bg-[#DBEFF3] animate-pulse"
              />
            ))}
          </div>
        ) : locations.length === 0 ? (
              ...
              {locations.map((loc) => (
                <div
                  ...
                  className={`group bg-white rounded-xl border p-5 flex flex-col gap-3 cursor-pointer transition-shadow hover:shadow-md ${
                    loc.isActive ? "border-[#DBEFF3]" : "border-gray-200 opacity-60"
                  }`}
                >
...
function DetailRow({
  ...
  return (
    <div className="flex items-start justify-between gap-6 py-2 border-b border-[#DBEFF3] last:border-b-0">
