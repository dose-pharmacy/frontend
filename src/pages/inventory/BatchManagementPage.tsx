...
export default function BatchManagementPage() {
  ...
  const nearExpiryCount = batches.filter(
    (batch) => batch.daysUntilExpiry >= 0 && batch.daysUntilExpiry <= 30,
  ).length
  ...
  return (
    <div className="flex flex-1 min-h-0 flex-col">
      ...
      <div className="flex-1 min-h-0 overflow-y-auto flex flex-col gap-6 p-6">
        {/* Error */}
        {loadError && (
                  ...
                  <tbody>
                    {batches.map((batch, index) => (
                      <tr
                        key={batch.id}
                        className={
                          index % 2 === 0 ? "bg-white" : "bg-[#E6ECE2]/30"
                        }
                      >
                        {/* Batch Number */}
