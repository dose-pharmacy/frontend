...
import { getBinCard, type BinCardResult, type StockTransactionDto } from "../../features/inventory/stockApi";
...
export default function BinCardPage() {
  ...
  useEffect(() => {
    ...
    getBinCard({ productId, locationId, batchId: batchId || undefined })
      ...
      .catch(err => setError(err instanceof Error ? err.message : "Failed to load bin card"))
  ...
  function handleExportCSV() {
    ...
    const rows = ledger.map((e) => `${e.date},${e.reference || ""},${e.transactionType},${e.in},${e.out},${e.runningBalance}`).join("\n");
      ...
      <PageHeader
        ...
        actions={
          <div className="flex gap-2">
            ...
            <Button onClick={handleExportCSV} disabled={!card || ledger.length === 0}>Export CSV</Button>
          </div>
        }
      />
      ...
      <div className="p-6 flex flex-col gap-6 overflow-y-auto">
        <div className="bg-white rounded-xl border border-[#DBEFF3] p-4 flex flex-col gap-4">
          ...
          <div className="grid sm:grid-cols-3 gap-4">
            <Select label="Product *" value={productId} onChange={(e) => { setProductId(e.target.value); setBatchId(""); setParams({}); }}>
              <option value="">Select Product...</option>
              {products.map((p) => <option key={p.id} value={p.id}>{p.name}</option>)}
            </Select>
            <Select label="Location *" value={locationId} onChange={(e) => { setLocationId(e.target.value); setParams({}); }}>
              <option value="">Select Location...</option>
              {locations.map((l) => <option key={l.id} value={l.id}>{l.name}</option>)}
            </Select>
            <Select label="Batch (Optional)" value={batchId} onChange={(e) => { setBatchId(e.target.value); setParams({}); }} disabled={!productId}>
        ...
        {!productId || !locationId ? (
          ...
          <div className="p-6 space-y-3 animate-pulse">{[...Array(3)].map((_, i) => <div key={i} className="h-20 bg-[#DBEFF3] rounded-lg" />)}</div>
        ) : card ? (
          <div className="flex flex-col gap-6">
            ...
            <div className="flex flex-wrap gap-3 items-end">
              <div className="flex-1 min-w-[150px]">
                <label className="text-xs font-medium text-[#666666] block mb-1">From Date</label>
                <input type="date" value={fromDate} onChange={(e) => setFromDate(e.target.value)} className="w-full rounded-xl border border-[#ABDBE3] px-3 py-2 text-sm focus:border-[#49B0C1] focus:outline-none" />
              </div>
              <div className="flex-1 min-w-[150px]">
                <label className="text-xs font-medium text-[#666666] block mb-1">To Date</label>
                <input type="date" value={toDate} onChange={(e) => setToDate(e.target.value)} className="w-full rounded-xl border border-[#ABDBE3] px-3 py-2 text-sm focus:border-[#49B0C1] focus:outline-none" />
              </div>
              <div className="flex-1 min-w-[150px]">
                <label className="text-xs font-medium text-[#666666] block mb-1">Movement Type</label>
                <select value={typeFilter} onChange={(e) => setTypeFilter(e.target.value)} className="w-full rounded-xl border border-[#ABDBE3] px-3 py-2 text-sm focus:border-[#49B0C1] focus:outline-none">
              ...
              {hasFilters && (
                <button onClick={() => { setFromDate(""); setToDate(""); setTypeFilter(""); }} className="text-xs font-semibold text-[#49B0C1] hover:underline pb-2">
                  Clear
                </button>
              )}
            </div>
              ...
              <div className="overflow-x-auto max-h-[500px] overflow-y-auto">
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {ledger.length === 0 ? (
                      <tr>
                        <td colSpan={7} className="px-4 py-8 text-center text-sm text-[#999]">No movements in the selected range.</td>
                      </tr>
                    ) : (
                      ledger.map((t, i) => (
                        <tr key={t.transactionId} className={i % 2 === 0 ? "bg-white" : "bg-[#DBEFF3]/20"}>
                          <td className="px-4 py-3 text-[#666666] whitespace-nowrap text-xs">{fmtDate(t.date)}</td>
                          ...
                          <td className="px-4 py-3 text-right font-semibold text-[#333333]">{t.runningBalance.toLocaleString()}</td>
