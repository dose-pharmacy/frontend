...
export default function InventoryDashboardPage() {
  ...
  const [expiringSoonBatches, setExpiringSoonBatches] =
    useState<ExpiryBatchDto[]>([])
    ...
    void (async () => {
      try {
        ...
        const ledgers = await Promise.all(
          sampleIds.map((id) =>
            getProductTransactions(id, { page: 1, limit: 5 }).catch(() => ({
              data: [] as StockTransactionDto[],
            })),
          ),
        )
        if (cancelled) return
                  ...
                  <div className="flex items-center justify-between px-5 py-3">
                    <div className="flex items-center gap-3">
                      ...
                      <span className="text-sm text-[#333333]">
                        <span className="font-semibold">{kpis.outOfStock}</span>{" "}
                        product{kpis.outOfStock !== 1 ? "s" : ""} out of stock
                      </span>
                    </div>
                    <button
                ...
                {kpis.lowStock > 0 && (
                  <div className="flex items-center justify-between px-5 py-3">
                    <div className="flex items-center gap-3">
                      ...
                      <span className="text-sm text-[#333333]">
                        <span className="font-semibold">{kpis.lowStock}</span>{" "}
                        product{kpis.lowStock !== 1 ? "s" : ""} low in stock
                      </span>
                    </div>
                    <button
                  ...
                  <div className="flex items-center justify-between px-5 py-3">
                    <div className="flex items-center gap-3">
                      ...
                      <span className="text-sm text-[#333333]">
                        ...
                        batch{kpis.expiringSoon !== 1 ? "es" : ""} expiring
                        within 30 days
                      </span>
                    </div>
                    <button
                      onClick={() => navigate("/inventory/batches-expiry")}
                      className="inline-flex items-center gap-0.5 text-xs font-semibold text-[#7A9076] hover:underline"
                    >
                      View <ChevronRight className="h-3.5 w-3.5" />
                    </button>
                  ...
                  >
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-[#666666]">
                        ...
                        {i.product.baseUnit?.name ??
                          i.product.baseUnit?.symbol ??
                          "unit"}
                        s
                      </p>
                    </div>
                    <div className="text-right ml-4 flex-shrink-0">
                      ...
                      <StatusChip
                        ...
                        tone={i.currentStock <= 0 ? "red" : "amber"}
                      />
                    </div>
                  </div>
                ))}
              ...
              <table className="w-full text-sm border-collapse">
                ...
                <tbody>
                  {recentActivity.map((t, i) => {
                    const productName =
                      (t.product as { name?: string } | undefined)?.name ??
                      t.productId
                    const locationName =
                      (t.location as { name?: string } | undefined)?.name ?? "—"
                    const batchNumber = (t.batch as {
                      batchNumber?: string
                    } | undefined)?.batchNumber
                    const isIn = t.direction === "IN"
                    const qtyLabel = `${t.quantity.toLocaleString()}${
