...
export default function SalesReportPage() {
  ...
  const loadSales = useCallback(async () => {
    ...
    try {
      ...
      setError(e instanceof ReportsApiError ? e.message : "Failed to load sales report.");
    } finally {
      setLoading(false);
    }
  ...
  async function openDetail(sale: SaleDto) {
    ...
    try {
      ...
      setDetailError(e instanceof ReportsApiError ? e.message : "Failed to load sale detail.");
    } finally {
        ...
        <div className="bg-white rounded-xl border border-[#E6ECE2] overflow-hidden">
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center py-20 gap-3">
              ...
              <p className="text-sm text-[#666666]">No transactions match the selected period and filters.</p>
            </div>
          ) : (
            <>
                  ...
                  <thead>
                    <tr className="bg-[#E6ECE2] text-left">
                      {[
                        ...
                        <th key="location" className="px-4 py-3 font-semibold text-[#333333]">Location</th>,
                        <th key="cashier" className="px-4 py-3 font-semibold text-[#333333]">Cashier</th>,
                        <th key="items" className="px-4 py-3 font-semibold text-[#333333] text-right">Items</th>,
                        <th key="discount" className="px-4 py-3 font-semibold text-[#333333] text-right">Discount</th>,
                        <th key="action" className="px-4 py-3 font-semibold text-[#333333] text-right">Actions</th>,
                      ]}
                    </tr>
                  </thead>
                  <tbody>
      ...
      <Modal open={detailOpen} title={detailSale ? `Sale ${detailSale.saleNumber}` : "Sale Detail"} onClose={closeDetail} size="lg">
        {detailLoading ? (
          ...
          <div className="flex flex-col items-center justify-center py-10 gap-4">
            <p className="text-sm text-red-600 bg-red-50 border border-red-100 rounded-xl px-4 py-3 text-center">{detailError}</p>
          ...
          <div className="flex flex-col gap-5">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
              {[
                ["Date", fmtDateTime(detailSale.completedAt ?? detailSale.createdAt)],
                ["Location", detailLocation?.name ?? "—"],
                ["Cashier", detailCashier?.name ?? "—"],
                ["Total", fmtMoney(detailSale.totalAmount)],
                ["Paid", detailSale.paidAmount != null ? fmtMoney(detailSale.paidAmount) : "—"],
            ...
            <div>
              <h3 className="text-sm font-bold text-[#333333] mb-2">Products</h3>
          ...
          <p className="text-sm text-[#666666] py-8 text-center">No detail available for this sale.</p>
        )}
      </Modal>
    </div>
  );
