...
function definitionLabel(def: SlowMovingConfigDto | SlowMovingDto): string {
  ...
  return (
    DEFINITION_OPTIONS.find((o) => o.value === def.definitionType)?.label ??
    def.definitionType
  )
}
...
function StatusBadge({ flagged }: { flagged: boolean }) {
  return flagged ? (
    <StatusChip label="Flagged" tone="red" />
  ) : (
    <StatusChip label="Not Flagged" tone="green" />
  )
}
...
export default function SlowMovingPage() {
    ...
    try {
      ...
      setReportError(
        e instanceof ReportsApiError
          ? e.message
          : "Failed to load slow-moving report.",
      )
    } finally {
  ...
  const loadConfigs = useCallback(async () => {
    ...
    try {
      const res = await getSlowMovingConfigs({
        page: configPage,
        limit: PAGE_SIZE,
      })
      setConfigs(res.data)
  ...
  const [evaluateResult, setEvaluateResult] =
    useState<SlowMovingEvaluateResult | null>(null)
  ...
  async function handleEvaluate() {
    ...
    try {
      ...
      setEvaluateError(
        e instanceof ReportsApiError ? e.message : "Evaluation failed.",
      )
    } finally {
      setEvaluating(false)
    }
  ...
  const [editingConfig, setEditingConfig] =
    useState<SlowMovingConfigDto | null>(null)
  const [formProductId, setFormProductId] = useState("")
  const [formDefinition, setFormDefinition] =
    useState<SlowMovingDefinitionType>("DAYS_30")
  ...
  async function handleSaveConfig() {
    const customDays =
      formDefinition === "CUSTOM" ? Number(formCustomDays) : null
    if (
  ...
  // ── Delete config ───────────────────────────────────────────────────────────
  const [deleteTarget, setDeleteTarget] = useState<SlowMovingConfigDto | null>(
    null,
  )
            ...
            <div className="flex flex-wrap gap-x-6 gap-y-1 mt-1 text-xs text-[#666666]">
              <span>
                ...
                <span className="font-bold text-[#333333]">
                  {fmtNumber(evaluateResult.evaluated)}
                </span>
              </span>
              <span>
                Flagged:{" "}
                <span className="font-bold text-red-600">
                  {fmtNumber(evaluateResult.flagged)}
                </span>
              </span>
              <span>
                Unflagged:{" "}
                <span className="font-bold text-green-600">
                  {fmtNumber(evaluateResult.unflagged)}
                </span>
              </span>
              <span>
                Skipped:{" "}
                <span className="font-bold text-[#333333]">
                  {fmtNumber(evaluateResult.skipped)}
                </span>
              </span>
          ...
          <div className="flex flex-wrap gap-3 items-end">
            ...
            <div className="flex flex-col gap-1.5 flex-1 min-w-[140px]">
              ...
              <select
                ...
                onChange={(e) => {
                  setDefinitionType(e.target.value)
                  setReportPage(1)
                }}
                className={selectClass}
              >
        ...
        <div className="bg-white rounded-xl border border-[#E6ECE2] overflow-hidden">
          <div className="px-5 py-4 border-b border-[#E6ECE2]">
            ...
            <p className="text-xs text-[#666666] mt-0.5">
              Products that have not sold within their configured threshold.
            </p>
          </div>
          {reportLoading ? (
            ...
            <div className="flex flex-col items-center justify-center py-20 gap-3">
              ...
              <p className="text-sm text-[#666666] max-w-sm text-center">
                Run the analysis to evaluate products against the configured
                thresholds.
              </p>
              <Button onClick={handleEvaluate} loading={evaluating}>
                Run Analysis
                  ...
                  <tbody>
                    {reportRows.map((row, i) => (
                      ...
                      >
                        ...
                        <td className="px-4 py-3 text-[#666666]">
                          {fmtNumber(row.thresholdDays)} days
                        </td>
                        <td className="px-4 py-3 text-[#666666] whitespace-nowrap">
                          {fmtDate(row.lastSaleAt)}
                        </td>
                        <td className="px-4 py-3 text-[#666666]">
                          {row.daysSinceLastSale != null
                            ? fmtNumber(row.daysSinceLastSale)
                            : "—"}
                        </td>
                        <td className="px-4 py-3">
                          <StatusBadge flagged={row.isFlagged} />
                        </td>
                        <td className="px-4 py-3 text-right text-[#666666]">
                          {row.stockQuantity != null
                            ? fmtNumber(row.stockQuantity)
                            : "—"}
                        </td>
                        <td className="px-4 py-3 text-right text-[#666666]">
                          {row.stockValue != null
        ...
        <div className="bg-white rounded-xl border border-[#E6ECE2] overflow-hidden">
          <div className="px-5 py-4 border-b border-[#E6ECE2] flex items-center justify-between flex-wrap gap-2">
            <div>
              ...
              <p className="text-xs text-[#666666] mt-0.5">
                Per-product slow-moving thresholds.
              </p>
            </div>
          ...
          {configsLoading ? (
            ...
            <div className="flex flex-col items-center justify-center py-16 gap-3">
              ...
              <p className="text-sm text-[#666666]">
                Add a configuration to set slow-moving thresholds per product.
              </p>
              <Button onClick={openCreate}>+ Add Configuration</Button>
            </div>
          ) : (
                  ...
                  <tbody>
                    {configs.map((cfg, i) => (
                      ...
                      >
                        ...
                        <td className="px-4 py-3">
                          <StatusBadge flagged={cfg.isFlagged ?? false} />
                        </td>
                        <td className="px-4 py-3 text-[#666666] whitespace-nowrap">
        ...
        <div className="flex flex-col gap-4">
              ...
              <>
                <div className="relative mb-2">
                  ...
                  <input
                    ...
                    className="w-full rounded-lg border border-[#C6D4BF] bg-white pl-9 pr-3.5 py-2.5 text-sm text-[#333333] placeholder:text-[#999] focus:border-[#B6C8AF] focus:outline-none focus:ring-2 focus:ring-[#B6C8AF]/20 transition-all"
                  />
                </div>
                <select
                  value={formProductId}
          ...
          <div>
            ...
            <select
              ...
              onChange={(e) =>
                setFormDefinition(e.target.value as SlowMovingDefinitionType)
              }
              className="w-full rounded-xl border border-[#C6D4BF] px-3.5 py-2.5 text-sm bg-white focus:border-[#B6C8AF] focus:outline-none"
            >
              {DEFINITION_OPTIONS.map((o) => (
