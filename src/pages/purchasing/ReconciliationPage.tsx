...
export default function ReconciliationPage() {
  ...
  function updateResolveItem(
    itemId: string,
    field: keyof ResolveItemState,
    value: string | number,
  ) {
        ...
        <div className="flex-1 flex items-center justify-center">
          <div className="text-center">
            ...
            <button
              ...
              className="text-[#7A9076] hover:underline text-sm"
            >
              Retry
            </button>
          </div>
  ...
  const canConfirm =
    receipt.status === "MATCHED" || receipt.status === "RESOLVED"
        ...
        actions={
          <div className="flex items-center gap-3">
            <StatusChip
              ...
              tone={STATUS_BADGE[receipt.status] ?? "gray"}
            />
        ...
        <div className="px-4 sm:px-6 py-4 bg-[#E6ECE2]">
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            <div className="bg-white rounded-xl p-4 text-center border border-[#C6D4BF]/30">
              ...
              <p className="text-lg font-bold text-[#333333] mt-1">
                    ...
                    >
                      ...
                      <td className="px-3 py-2.5 font-medium text-[#333333]">
                        {item.purchaseOrderItem?.product?.name ??
                          `Product (${item.purchaseOrderItem?.productId?.slice(0, 8) ?? "?"})`}
                      </td>
          ...
          <div className="px-4 sm:px-6 py-4">
            <div className="bg-yellow-50 border border-yellow-200 rounded-xl p-4">
              ...
              <p className="text-sm text-yellow-700 mb-4">
                This receipt has a discrepancy. Review and adjust quantities
                below, then resolve before confirming.
              </p>
                ...
                <table className="w-full text-sm min-w-[600px] mb-4">
                  <thead>
                    <tr className="bg-yellow-100">
                      {[
                        ...
                        <th
                          key={h}
                          className="px-3 py-2 text-left font-semibold text-yellow-800"
                        >
                          {h}
                        </th>
                      ))}
                  ...
                  <tbody>
                    {resolveItems.map((item, i) => {
                      const originalItem = receipt.items.find(
                        (ri) => ri.id === item.id,
                      )
                      return (
                        <tr
                          key={item.id}
                          className={
                            i % 2 === 0 ? "bg-white" : "bg-yellow-50/50"
                          }
                        >
                          <td className="px-3 py-2 font-medium text-[#333333]">
                            {originalItem?.purchaseOrderItem?.product?.name ??
                              `Item ${i + 1}`}
                          </td>
                          <td className="px-3 py-2">
                            <input
                              ...
                              className="w-16 rounded border border-yellow-300 px-2 py-1 text-sm"
                            />
                          </td>
                          <td className="px-3 py-2">
                            <input
                              ...
                              className="w-24 rounded border border-yellow-300 px-2 py-1 text-xs"
                            />
                          </td>
                          <td className="px-3 py-2">
                            <input
              ...
              <div className="mb-3">
                ...
                <textarea
                  ...
                  className="w-full rounded-lg border border-yellow-300 bg-white px-3 py-2 text-sm focus:outline-none resize-none"
                />
              </div>
          ...
          <div className="px-4 sm:px-6 py-4">
            <div className="bg-green-50 border border-green-200 rounded-xl p-4">
              ...
              <p className="text-sm text-green-700 mb-3">
                Confirming will update inventory stock levels with the actual
                quantities received.
              </p>
              <button
                onClick={handleConfirm}
