...
export default function SupplierInvoicesPage() {
  ...
  async function handleDelete(id: string) {
    ...
    try {
      ...
      showToast(
        e instanceof SupplierInvoicesApiError ? e.message : "Delete failed",
      )
    }
  }
  ...
  async function handleRecordPayment() {
    ...
    setPaying(true)
    try {
      await recordInvoicePayment(payingId, {
        amount: amountNum,
      ...
      setPayError(
        e instanceof SupplierInvoicesApiError ? e.message : "Payment failed",
      )
    } finally {
      setPaying(false)
    }
                    ...
                    {invoices.map((inv, i) => (
                      ...
                      >
                        <td className="px-4 py-3">
                          <button
                            onClick={() =>
                              navigate(`/purchasing/invoices/${inv.id}`)
                            }
                        ...
                        <td className="px-4 py-3 text-right font-semibold text-[#333333]">
                          {fmtMoney(inv.invoiceAmount)}
                        </td>
                        <td className="px-4 py-3 text-right text-red-600 font-semibold">
                          {fmtMoney(invoiceOutstanding(inv))}
                        </td>
                        <td className="px-4 py-3">
                          <StatusChip
                            label={inv.status}
                            tone={STATUS_BADGE[inv.status] ?? "gray"}
                          />
                        </td>
                        <td className="px-4 py-3">
                          <div className="flex items-center gap-2">
                            ...
                            {inv.status !== "PAID" && (
                              <button
                                onClick={() =>
                                  openPayModal(inv.id, invoiceOutstanding(inv))
                                }
                                className="text-xs font-semibold text-[#7A9076] hover:underline whitespace-nowrap"
                              >
                                Pay
