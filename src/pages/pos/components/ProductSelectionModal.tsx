...
export default function ProductSelectionModal({ product, onAdd, onClose }: Props) {
  const availableUnits = product.units.filter((u) => u.stock > 0);
  const [selectedUnit, setSelectedUnit] = useState<POSUnit>(availableUnits[0] ?? product.units[0]);
  const [quantity, setQuantity] = useState(1);

  const maxQty = selectedUnit?.stock ?? 0;
  const totalPrice = (selectedUnit?.price ?? 0) * quantity;
  const isOutOfStock = product.status === "out_of_stock" || !selectedUnit || selectedUnit.stock === 0;
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" role="dialog" aria-modal aria-labelledby="ps-title">
      ...
      <div className="relative bg-white rounded-xl shadow-2xl w-full max-w-lg flex flex-col overflow-hidden animate-in fade-in slide-in-from-bottom-4 duration-300">
        ...
        <div className="bg-[#49B0C1] px-6 py-4 flex items-center justify-between">
          <h2 id="ps-title" className="text-lg font-bold text-white">Add to Sale</h2>
        ...
        <div className="bg-[#DBEFF3] px-6 py-4 flex items-center gap-4">
          <div className="h-16 w-16 rounded-xl bg-white/70 flex items-center justify-center text-[#49B0C1] flex-shrink-0">
            <ProductIcon type={product.icon} size={32} />
          </div>
          <div className="flex-1 min-w-0">
            <h3 className="font-bold text-[#333333] text-lg leading-tight">{product.name}</h3>
            <p className="text-sm text-[#666666]">{product.brand}</p>
            <StockBadge status={product.status} stock={selectedUnit?.stock} />
          </div>
          <div className="text-right flex-shrink-0">
            <p className="text-xl font-bold text-[#49B0C1]">{fmt(selectedUnit?.price ?? 0)}</p>
        ...
        <div className="p-6 flex flex-col gap-5">
          ...
          <div>
            ...
            {/* Stock bar */}
            <div className="mt-2 h-1.5 rounded-full bg-[#DBEFF3] overflow-hidden w-48">
              <div
                className={`h-full rounded-full transition-all ${quantity / maxQty > 0.9 ? "bg-red-400" : quantity / maxQty > 0.5 ? "bg-yellow-400" : "bg-green-400"}`}
          ...
          <div className="flex items-center justify-between rounded-lg bg-[#DBEFF3] px-4 py-3">
            <span className="text-sm text-[#666666]">Total Price</span>
            <span className="text-xl font-bold text-[#49B0C1]">{fmt(totalPrice)}</span>
          </div>

          {/* Stock impact warning */}
          {quantity > 0 && selectedUnit && (
            <div className="flex items-start gap-2 rounded-lg border border-yellow-300 bg-yellow-50 px-4 py-2.5 text-xs text-yellow-800">
              <span className="mt-0.5" aria-hidden>⚠</span>
              <span>This will deduct {quantity} {selectedUnit.name}(s) from available stock.</span>
            </div>
          )}
        </div>
        ...
        <div className="border-t border-[#DBEFF3] px-6 py-4 flex gap-3 justify-end bg-white">
          <button
            onClick={onClose}
            className="rounded-lg px-6 py-2.5 text-sm font-semibold text-white bg-red-500 hover:bg-red-600 transition-colors"
          >
            Cancel
          </button>
          <button
...
function StockBadge({ status, stock }: { status: string; stock?: number }) {
  if (status === "in_stock") return <span className="inline-flex items-center gap-1 text-xs font-medium text-green-700 mt-1">✓ In Stock{stock !== undefined ? `: ${stock} units` : ""}</span>;
  if (status === "low_stock") return <span className="inline-flex items-center gap-1 text-xs font-medium text-yellow-700 mt-1">⚠ Low Stock{stock !== undefined ? `: ${stock} units` : ""}</span>;
  return <span className="inline-flex items-center gap-1 text-xs font-medium text-gray-500 mt-1">✕ Out of Stock</span>;
}
