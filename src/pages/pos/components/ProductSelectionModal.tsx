...
export default function ProductSelectionModal({
  ...
  const isOutOfStock =
    product.status === "out_of_stock" || product.availableStock <= 0
    ...
    >
      ...
      <div className="relative bg-white rounded-xl shadow-2xl w-full max-w-lg flex flex-col overflow-hidden animate-in fade-in slide-in-from-bottom-4 duration-300">
        ...
        <div className="bg-white border-b border-[#E6ECE2] px-6 py-4 flex items-center justify-between">
          <h2 id="ps-title" className="text-base font-bold text-[#333333]">
            Add to Sale
          </h2>
          <button
            onClick={onClose}
            className="text-[#333333]/80 hover:text-[#333333] transition-colors"
            aria-label="Close"
          >
        ...
        <div className="bg-[#E6ECE2] px-6 py-4 flex items-center gap-4">
          ...
          <div className="text-right flex-shrink-0">
            <p className="text-lg font-bold text-[#7A9076]">
              {fmt(selectedUnit?.price ?? 0)}
            </p>
                ...
                {product.units.map((u) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <span className="text-xs text-[#7A9076] font-semibold">
                        {fmt(u.price)}
                      </span>
          ...
          <div>
            ...
            <div className="flex items-center gap-3">
              <button
                onClick={decrement}
                disabled={quantity <= 1}
                className="h-9 w-9 rounded-full bg-[#C6D4BF] text-[#333333] flex items-center justify-center hover:bg-[#B5C8AE] transition-colors disabled:opacity-40 text-xl font-bold leading-none"
              ...
              <button
                ...
                className="h-9 w-9 rounded-full bg-[#B6C8AF] text-[#333333] flex items-center justify-center hover:bg-[#A0B59C] transition-colors disabled:opacity-40 text-xl font-bold leading-none"
        ...
        <div className="border-t border-[#E6ECE2] px-6 py-4 flex gap-3 justify-end bg-white">
          <button
            onClick={onClose}
            className="rounded-lg px-6 py-2.5 text-sm font-semibold text-[#333333] bg-[#C6D4BF] hover:bg-[#B5C8AE] transition-colors"
          >
            Cancel
          </button>
          <button
...
function StockBadge({
  ...
  if (status === "in_stock")
    return (
      <StatusChip label={`In Stock · ${availableStock} units`} tone="green" />
    )
  if (status === "low_stock")
    return (
      <StatusChip label={`Low Stock · ${availableStock} units`} tone="amber" />
    )
  return <StatusChip label="Out of Stock" tone="gray" />
}
    ...
    return (
      ...
      >
        <path
          strokeLinecap="round"
          strokeLinejoin="round"
          d="M12 3C12 3 5.25 10.5 5.25 14.25a6.75 6.75 0 0013.5 0C18.75 10.5 12 3 12 3z"
        />
      </svg>
    )
  if (type === "shield")
